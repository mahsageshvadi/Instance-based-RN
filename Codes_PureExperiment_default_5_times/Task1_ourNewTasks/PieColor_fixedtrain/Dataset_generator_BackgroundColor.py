...
def GenerateOnePieChart(num, size = 100, random_color = True):
    ...
    subImages = [np.full((size, size, 3), background_color, dtype=np.uint8) for i in range(config.max_obj_num)]
    ...
    for i in range(num):
        ...
        cv2.ellipse(image, center, (r, r), 270, -_cur_start_angle, -_cur_end_angle, colors[i]*255, -1)
        cv2.ellipse(subImages[i],center,(r,r),270,-_cur_start_angle,-_cur_end_angle,colors[i]*255, -1)
    ...
    for t in range(len(subImages)):
        ...
        subImages[t] = subImages[t] + noises
        _min = 0.0
        _max = subImages[t].max()
        subImages[t] = subImages[t] -_min
        subImages[t] = subImages[t] /(_max - _min)
...
def GenerateDatasetVGG(flag, image_num):
    ...
    isRandomColor = False if flag == 'train' else True
...
def GenerateDatasetIRNm(flag, image_num):
    ...
    for i in range(image_num):
        ...
        _, subimages, featureVector = GenerateOnePieChart(
                num=np.random.randint(min_num_obj, max_num_obj + 1), random_color=isRandomColor)
        featureVector = np.array(featureVector)
