...
max_train_obj = 6    # 6
min_test_obj = 7
max_test_obj = config.max_obj_num     # 9
...
def GenerateOneBarChart(num, size = config.image_width):
    ...
    for i in range(num):
        ...
        cv2.rectangle(image,(sx,sy),(ex,ey),0,1)
        cv2.rectangle(subImages[i],(sx,sy),(ex,ey),0,1)
        sx = ex + spaceWidth
...
def GenerateDatasetVGG(flag, image_num):
    ...
    for i in range(image_num):
        ...
        image, _, featureVector = GenerateOneBarChart(num=np.random.randint(min_num_obj, max_num_obj + 1))
